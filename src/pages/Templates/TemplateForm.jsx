...
const REQUIRED_PLACEHOLDERS = ["userName", "resumeLink", "userEmail"];
...
const TemplateForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.body.trim()) {
      ...
      const missingRequired = REQUIRED_PLACEHOLDERS.filter(
        (p) => !formData.placeholders.includes(p)
      );
  ...
  return (
    <motion.div
      ...
      className="min-h-screen flex bg-gradient-to-br from-purple-950 via-gray-700 to-purple-950 text-white"
    >
      <div className="w-[65%] flex-shrink-0 p-4">
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur-sm border border-purple-800 rounded-xl shadow-xl p-6 ">
      ...
      {/* Right sidebar */}
      <div className="w-[35%] bg-gray-900 bg-opacity-40 border-l border-purple-800 p-6 flex flex-col h-screen">
        ...
        <div className="space-y-4 flex-1">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {["jobId", "hrName", "companyName"].map((placeholder) => (
                  <div key={placeholder}>
          ...
          {formData.placeholders.length > 0 &&
            (() => {
              const requiredDetected = formData.placeholders.filter((p) =>
                REQUIRED_PLACEHOLDERS.includes(p)
              );
              const optionalDetected = formData.placeholders.filter(
                (p) => !REQUIRED_PLACEHOLDERS.includes(p)
              );
              const totalVisible = expandedDetected
                ? formData.placeholders.length
                : Math.min(6, formData.placeholders.length);
              const hasMore = formData.placeholders.length > 6;

              const visiblePlaceholders = expandedDetected
                ? [...requiredDetected, ...optionalDetected]
                : [
                    ...requiredDetected,
                    ...optionalDetected.slice(
                      0,
                      Math.max(0, totalVisible - requiredDetected.length)
                    ),
                  ];
